...
class AssistantManager:
    ...
    async def update_assistant_file(
            self, assistant_id: str, file_id: str,
            file_path: str, file_name: str, files: List
    ):
        await self.client.files.delete(file_id)

        open_ai_file = await self.client.files.create(
            file=open(file_path, "rb"), purpose='assistants'
        )

        await self.client.beta.assistants.update(
            assistant_id, file_ids=[open_ai_file.id, *files]
        )
        await FileId.objects.filter(key=file_name).aupdate(value=open_ai_file.id)
